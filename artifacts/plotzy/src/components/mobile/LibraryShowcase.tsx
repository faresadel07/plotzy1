...
export function LibraryShowcase({ ar, dark = false, kicker, kickerAr, title, titleAr, sub, subAr, cta, ctaAr, href, covers, note, noteAr }: LibraryShowcaseProps) {
  ...
  return (
    <section dir={ar ? "rtl" : "ltr"} style={{ padding: "0 14px", marginBottom: 26, fontFamily: SF }}>
      ...
      >
        ...
        <p style={{ fontSize: ar ? 13.5 : 14, lineHeight: 1.65, color: dark ? "rgba(244,239,226,0.6)" : "#7b7366", maxWidth: 300, margin: `0 auto ${note ? 8 : 18}px` }}>
          {ar ? subAr : sub}
        </p>

        {note && (
          <div style={{ fontFamily: ar ? HAND_AR : HAND_EN, fontSize: ar ? 13.5 : 16, color: dark ? "rgba(244,239,226,0.5)" : "#8a8070", marginBottom: 14, transform: "rotate(-1deg)" }}>
            {ar ? (noteAr || note) : note}
          </div>
        )}
        ...
        <button
          ...
          style={{
            width: "100%",
            // The browns Faris asked for: paper button on espresso
            // sections, espresso button on paper sections.
