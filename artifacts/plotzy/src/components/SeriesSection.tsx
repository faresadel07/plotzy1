...
import {
  BookOpen, Plus, Pencil, Trash2, X, Check, ChevronRight, ChevronLeft,
  Library, GripVertical, Layers
} from "lucide-react";
...
function MiniCover({ book, onClick }: { book: { id: number; title: string; coverImage?: string | null; spineColor?: string | null }; onClick: () => void }) {
  const spine = book.spineColor ?? "#7c3aed";
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      {/* Ordinal tooltip (just a nice touch) */}
...
// Manage books dialog — add / remove books from a series
          ...
          {candidates.map((book) => {
            ...
            return (
              ...
              >
                ...
                <span className={`flex-shrink-0 w-5 h-5 rounded-full border flex items-center justify-center transition-all ${inSeries ? "bg-violet-500 border-violet-500" : "border-white/20"}`}>
                  {inSeries && <Check className="w-3 h-3 text-white" />}
                </span>
              </button>
            );
          })}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-5">
        <div className="w-8 h-8 rounded-lg bg-violet-500/15 border border-violet-500/25 flex items-center justify-center flex-shrink-0 mt-0.5">
          ...
          {editingName ? (
            <form onSubmit={(e) => { e.preventDefault(); saveName(); }} className="flex items-center gap-2">
              <Input
                ...
                className="h-7 text-sm bg-white/5 border-white/20 text-white focus:border-violet-500 px-2"
                onClick={(e) => e.stopPropagation()}
              />
              <button type="submit" className="p-1 text-violet-400 hover:text-violet-300"><Check className="w-4 h-4" /></button>
        ...
        {showConfirmDelete && (
          ...
          >
            ...
            <div className="flex gap-2 mt-1">
              ...
              <Button size="sm" onClick={handleDelete} className="bg-red-500/80 hover:bg-red-500 text-white text-xs h-8 border-0">
                Delete Series
              </Button>
            </div>
          </motion.div>
      ...
      <DialogContent className="bg-[#111] border border-white/10 text-white max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-white text-base font-semibold flex items-center gap-2">
            <Layers className="w-4 h-4 text-violet-400" /> New Book Series
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-1">
          <div>
            ...
            <Input
              ...
              className="bg-white/5 border-white/15 text-white placeholder:text-white/20 focus:border-violet-500"
            />
          </div>
          <div>
            <label className="text-xs text-white/40 uppercase tracking-wider font-semibold block mb-1.5">Description <span className="text-white/20 normal-case tracking-normal font-normal">(optional)</span></label>
            <Textarea
              placeholder="A short description of this series…"
              value={desc}
              onChange={(e) => setDesc(e.target.value)}
              rows={2}
              className="bg-white/5 border-white/15 text-white placeholder:text-white/20 focus:border-violet-500 resize-none text-sm"
            />
          </div>
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" disabled={!name.trim() || create.isPending} className="flex-1 bg-violet-600 hover:bg-violet-500 text-white text-sm border-0">
        ...
        {(!seriesList || seriesList.length === 0) ? (
          ...
          >
            <div className="w-12 h-12 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center mx-auto mb-4">
              <Layers className="w-6 h-6 text-violet-400/70" />
            </div>
            ...
            <Button
              onClick={() => setCreateOpen(true)}
              className="bg-violet-600/80 hover:bg-violet-600 text-white border-0 gap-2 text-sm"
            >
              <Plus className="w-3.5 h-3.5" />
