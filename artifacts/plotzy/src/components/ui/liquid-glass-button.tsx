  ...
  return (
    <>
      <Comp
        ...
        {...props}
      >
        <div className="absolute top-0 left-0 z-0 h-full w-full rounded-[var(--btn-radius,9999px)] 
        ...
        <div
          className="absolute top-0 left-0 isolate -z-10 h-full w-full overflow-hidden rounded-[var(--btn-radius,9999px)]"
...
const metalButtonVariants = (
  ...
  return {
    wrapper: cn(
      "relative inline-flex transform-gpu rounded-[var(--btn-radius,9999px)] p-[1.25px] will-change-transform mt-0",
      colors.outer,
    ),
    wrapperStyle: {
      transform: isPressed
    ...
    inner: cn(
      "absolute inset-[1px] transform-gpu rounded-[calc(var(--btn-radius,9999px)-1.25px)] will-change-transform",
      colors.inner,
    ),
    innerStyle: {
      transition: transitionStyle,
    ...
    button: cn(
      "relative z-10 m-[1px] inline-flex h-11 transform-gpu cursor-pointer items-center justify-center overflow-hidden rounded-[calc(var(--btn-radius,9999px)-1.25px)] px-6 py-2 text-sm leading-none font-semibold will-change-transform outline-none w-full",
      colors.button,
      colors.textColor,
      colors.textShadow,
    ),
...
const ShineEffect = ({ isPressed }: { isPressed: boolean }) => {
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-0 z-20 overflow-hidden transition-opacity duration-300 rounded-[var(--btn-radius,9999px)]",
    ...
    <div className={variants.wrapper} style={variants.wrapperStyle}>
      ...
      >
        ...
        {isHovered && !isPressed && !isTouchDevice && (
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t rounded-[calc(var(--btn-radius,9999px)-1.25px)] from-transparent to-white/5" />
        )}
      </button>
    </div>
  );
