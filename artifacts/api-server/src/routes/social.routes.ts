...
router.get("/api/me/export-data", sensitiveAuthLimiter, async (req, res) => {
  try {
    ...
    // Format dispatch — `?format=pdf` returns the human-readable PDF
    // summary, anything else (including the legacy no-param call)
    // returns the technical JSON dump. Two formats share one endpoint
    // so the single rate limiter (sensitiveAuthLimiter, 5 req / 15
    // min) covers both buttons in the UI without us having to
    // coordinate two separate limit keys.
    const format = (typeof req.query.format === "string" && req.query.format.toLowerCase() === "pdf") ? "pdf" : "json";
    ...
    await logAuditEvent({
      ...
      targetType: "user",
      targetId: userId,
      details: { schemaVersion: data.schemaVersion, format },
      req,
    });
    ...
    if (format === "pdf") {
      // Language for PDF labels — comes from the client because we
      // don't persist a per-user language column. Default to English.
    ...
    res.setHeader("Content-Disposition", `attachment; filename="plotzy-export-${dateStamp}.json"`);
